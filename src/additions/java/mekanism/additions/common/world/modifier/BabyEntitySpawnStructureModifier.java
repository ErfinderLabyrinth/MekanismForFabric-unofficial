...
public record BabyEntitySpawnStructureModifier(BabyType babyType) implements StructureModifier {

    @Override
    public void modify(Holder<Structure> structure, Phase phase, StructureInfo.Builder builder) {
        if (phase == Phase.REMOVE && babyType.getConfig().shouldSpawn) {
            ...
            if (spawnOverrides != null && !babyType.getConfig().structureBlackList.contains(structureName)) {
                for (MobSpawnSettings.SpawnerData spawner : babyType.getSpawnersToAdd(spawnOverrides.getSpawns())) {
