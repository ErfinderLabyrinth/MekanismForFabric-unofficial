...
public class MekanismDatapackRegistryProvider extends BaseDatapackRegistryProvider {
    ...
    public MekanismDatapackRegistryProvider(FabricDataOutput output, CompletableFuture<HolderLookup.Provider> lookupProvider) {
    ...
    protected void configure(HolderLookup.Provider registries, Entries entries) {
        ...
        for (OreType type : EnumUtils.ORE_TYPES) {
            ...
            for (int vein = 0; vein < features; vein++) {
                ...
                entries.add(configuredFeature(name), configureOreFeature(oreVeinType, MekanismFeatures.ORE));
                entries.add(configuredFeature(name.withSuffix("_retrogen")), configureOreFeature(oreVeinType, MekanismFeatures.ORE_RETROGEN));
            }
        }
        entries.add(configuredFeature(Mekanism.rl("salt")), new ConfiguredFeature<>(MekanismFeatures.DISK.get(), new ResizableDiskConfig(
        ...
        for (MekanismDamageType damageType : MekanismDamageTypes.DAMAGE_TYPES.values()) {
            entries.add(damageType.key(), new DamageType(damageType.getMsgId(), damageType.exhaustion()));
        }

        //Robit Skin Registry Name
        entries.add(MekanismRobitSkins.BASE, makeRobitSkin(Mekanism.rl("robit"), 2));
        entries.add(MekanismRobitSkins.ALLAY, new AdvancementBasedRobitSkin(
                List.of(
                        Mekanism.rl("allay"),
                        Mekanism.rl("allay2")
                ),
        ...
        for (Map.Entry<RobitPrideSkinData, ResourceKey<RobitSkin>> entry : MekanismRobitSkins.PRIDE_SKINS.entrySet()) {
            ...
            entries.add(key, makeRobitSkin(key.location(), entry.getKey().getColor().length));
        }
    }
