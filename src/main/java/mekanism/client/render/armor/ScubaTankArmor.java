...
public class ScubaTankArmor implements ArmorRenderer, ResourceManagerReloadListener, IdentifiableResourceReloadListener {
    ...
    public void render(PoseStack matrix, MultiBufferSource renderer, ItemStack stack, LivingEntity entity, EquipmentSlot slot, int light, HumanoidModel<LivingEntity> baseModel) {
        ...
        if (baseModel.young) {
            ...
            renderTank(baseModel, matrix, renderer, light, OverlayTexture.NO_OVERLAY, stack.hasFoil());
            matrix.popPose();
        } else {
            renderTank(baseModel, matrix, renderer, light, OverlayTexture.NO_OVERLAY, stack.hasFoil());
        }
