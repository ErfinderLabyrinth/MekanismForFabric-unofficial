...
//    }
    ...
    //Slightly modified copy of ShearsDispenseItemBehavior#tryShearBeehive modified to not crash if the tagSupplier has a block that isn't a
    ...
    private boolean shearEntity(@Nullable EnergyStorage energyContainer, LivingEntity entity, @Nullable Player player, ItemStack stack, Level world, BlockPos pos) {
        Shearable target = (Shearable) entity;
        if (target.readyForShearing()) {
            if (!world.isClientSide) {
                target.shear(player.getSoundSource());
