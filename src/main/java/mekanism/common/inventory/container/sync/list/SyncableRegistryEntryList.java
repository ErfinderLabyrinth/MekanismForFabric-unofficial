...
public class SyncableRegistryEntryList<V> extends SyncableList<V> {
    ...
    public static <V> SyncableRegistryEntryList<V> create(Registry<V> registry, Supplier<@NotNull List<V>> getter, Consumer<@NotNull List<V>> setter) {
        return new SyncableRegistryEntryList<>(registry, getter, setter);
    }

    private final Registry<V> registry;

    private SyncableRegistryEntryList(Registry<V> registry, Supplier<@NotNull List<V>> getter, Consumer<@NotNull List<V>> setter) {
        super(getter, setter);
