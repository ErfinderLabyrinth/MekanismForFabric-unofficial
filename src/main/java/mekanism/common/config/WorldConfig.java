...
public class WorldConfig extends BaseMekanismConfig {
    ...
    public boolean enableRegeneration = false;
    public int userGenVersion = 0;
...
//              .defineInRange("userWorldGenVersion", 0, 0, Integer.MAX_VALUE));
        for (OreType ore : EnumUtils.ORE_TYPES) {
            ores.put(ore, new OreConfig(ore));
        }
        salt = new SaltConfig(2, 2, 3, 1);
        //builder.pop();
    ...
    public record OreVeinConfig(int perChunk, int maxVeinSize, float discardChanceOnAirExposure,
    ...
    public static class OreConfig {
        ...
        private boolean shouldVeinTypeGenerate = true;
        private List<OreVeinConfig> veinConfigs;

        private OreConfig(OreType oreType) {
            ...
            for (BaseOreConfig baseConfig : oreType.getBaseConfigs()) {
                ...
                veinBuilder.add(new OreVeinConfig(
                      /*() -> this.shouldGenerate && shouldVeinTypeGenerate,*/ baseConfig.perChunk(), baseConfig.maxVeinSize(), baseConfig.discardChanceOnAirExposure(), ConfigurableHeightRange.create(veinType, baseConfig)
        ...
        public boolean shouldGenerate() {
            return shouldGenerate && shouldVeinTypeGenerate;
        }
    }
    ...
    public static class SaltConfig {
        ...
        private SaltConfig(int perChunk, int baseRadius, int spread, int ySize) {
...
//            builder.pop();
            ...
            this.minRadius = baseRadius;
            this.maxRadius = spread;
            this.halfHeight = ySize;
        }
    }
}