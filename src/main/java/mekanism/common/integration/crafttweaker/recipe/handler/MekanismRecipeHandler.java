...
//                    KnownTag<Item> tagSupplier = CrTUtils.itemTags().tagSupplier(serializedIngredient.get(JsonConstants.TAG).getAsString());
//                    return amount == 1 ? tagSupplier.getCommandString() : tagSupplier.withAmount(amount).getCommandString();
//                }
...
//            JsonObject serialized = ingredient.serialize().getAsJsonObject();
//            //Note: Handled via implicit casts
//            return CrTUtils.fluidTags().tagSupplier(serialized.get(JsonConstants.TAG).getAsString())
...
//            JsonObject serialized = ingredient.serialize().getAsJsonObject();
//            KnownTag<CHEMICAL> tagSupplier = tagManager.tagSupplier(serialized.get(JsonConstants.TAG).getAsString());
//            long amount = serialized.getAsJsonPrimitive(JsonConstants.AMOUNT).getAsLong();
//            if (amount > 0 && amount <= Integer.MAX_VALUE) {
//                //Note: Handled via implicit casts
//                return tagSupplier.withAmount((int) amount).getCommandString();
//            }
//            //Tag with amount can only handle up to max int, so we have to do it explicitly if we have more
//            return crtClass + ".from(" + tagSupplier.getCommandString() + ", " + amount + ")";
