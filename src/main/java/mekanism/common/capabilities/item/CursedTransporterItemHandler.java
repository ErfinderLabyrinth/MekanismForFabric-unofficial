...
//
//public class CursedTransporterItemHandler extends SnapshotParticipant<CompoundTag> implements Storage<ItemVariant> {
//
...
//    private final Set<PendingItemStack> seenStacks = Collections.newSetFromMap(new IdentityHashMap<>());
//    private final Set<PendingItemStack> seenExecutedStacks = Collections.newSetFromMap(new IdentityHashMap<>());
...
//            return TransitRequest.simple(PendingItemStack.of(stack));
//        }
//        return TransitRequest.simple(PendingItemStack.of(stack.copyWithCount(limit)));
//    }
//
//    @NotNull
...
//        if (simulate) {
//            if (seenExecutedStacks.contains(itemStack) || !seenStacks.add(PendingItemStack.of(itemStack))) {
...
//            }
//        } else {
//            if (!seenExecutedStacks.add(PendingItemStack.of(itemStack))) {
//                //Failed because we have already seen it this tick (tried to add it OR it was one of the remainders we returned that couldn't be inserted further)
//                return itemStack;
//            }
...
//            }
//        }
//        PendingItemStack remainder = response.getRejected();
//
...
//                remainder = PendingItemStack.of(itemStack.copyWithCount(extra));
//            } else {
//                //Note: It is a new stack, so we can safely modify it, so add back the extra that we skipped attempting to insert
//                remainder = remainder.copyWithCount(remainder.amount() + extra);
//            }
//        }
//
//        if (!remainder.isEmpty()) {
...
//        FluidStack stack = insertItem(resource.toStack((int)maxAmount), false);
//        return maxAmount - stack.amount();
//    }
//
//    @Override
//    public long extract(ItemVariant resource, long maxAmount, TransactionContext transaction) {
//        updateSnapshots(transaction);
//        FluidStack stack = extract(maxAmount, Action.EXECUTE, AutomationType.EXTERNAL);
//        return stack.amount();
//    }
//
//    @Override
...
//        transporter.writeToNBT(nbt);
//        return nbt;
//    }
//
//    @Override
//    protected void readSnapshot(CompoundTag snapshot) {
//        transporter.readFromNBT(snapshot);
//    }
//}